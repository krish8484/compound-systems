...
def test_dot_product(scheduler_client, matrix1, matrix2):
    task = Task(taskId="0", taskDefintion="dot_product", taskData=json.dumps([matrix1, matrix2]).encode())
...
def test_mat_add(scheduler_client, matrix1, matrix2):
    task = Task(taskId="1", taskDefintion="mat_add", taskData=json.dumps([matrix1, matrix2]).encode())
    future = scheduler_client.SubmitTask(task)
...
def test_mat_subtract(scheduler_client, matrix1, matrix2):
    task = Task(taskId="2", taskDefintion="mat_subtract", taskData=json.dumps([matrix1, matrix2]).encode())
    future = scheduler_client.SubmitTask(task)
